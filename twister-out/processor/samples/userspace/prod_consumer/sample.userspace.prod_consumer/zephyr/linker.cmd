...
MEMORY
    ...
    QSPI : ORIGIN = 2415919104, LENGTH = 268435456 SRAM1 : ORIGIN = 805306368, LENGTH = 16384 SRAM2 : ORIGIN = 805322752, LENGTH = 16384 SRAM4 : ORIGIN = 939524096, LENGTH = 16384 DTCM : ORIGIN = 536870912, LENGTH = 131072 ITCM : ORIGIN = 0, LENGTH = 65536
...
__ramfunc_load_start = LOADADDR(.ramfunc);
 app_smem :
 {
  . = ALIGN(_region_min_align);;
  _app_smem_start = .;
  . = ALIGN(_region_min_align); . = ALIGN( 1 << LOG2CEIL(z_data_smem_shared_partition_bss_end - z_data_smem_shared_partition_part_start));
  z_data_smem_shared_partition_part_start = .;
  KEEP(*(data_smem_shared_partition_data*))
  z_data_smem_shared_partition_bss_start = .;
  KEEP(*(data_smem_shared_partition_bss*))
  z_data_smem_shared_partition_bss_end = .;
  . = ALIGN(_region_min_align); . = ALIGN( 1 << LOG2CEIL(z_data_smem_shared_partition_bss_end - z_data_smem_shared_partition_part_start));
  z_data_smem_shared_partition_part_end = .;
  ...
  *libc.a:*(.bss .bss.* .sbss .sbss.* COMMON COMMON.*)
  ...
  . = ALIGN(_region_min_align); . = ALIGN( 1 << LOG2CEIL(z_data_smem_app_b_partition_bss_end - z_data_smem_app_b_partition_part_start));
  z_data_smem_app_b_partition_part_start = .;
  KEEP(*(data_smem_app_b_partition_data*))
  z_data_smem_app_b_partition_bss_start = .;
  KEEP(*(data_smem_app_b_partition_bss*))
  z_data_smem_app_b_partition_bss_end = .;
  . = ALIGN(_region_min_align); . = ALIGN( 1 << LOG2CEIL(z_data_smem_app_b_partition_bss_end - z_data_smem_app_b_partition_part_start));
  z_data_smem_app_b_partition_part_end = .;
  . = ALIGN(_region_min_align); . = ALIGN( 1 << LOG2CEIL(z_data_smem_app_a_partition_bss_end - z_data_smem_app_a_partition_part_start));
  z_data_smem_app_a_partition_part_start = .;
  KEEP(*(data_smem_app_a_partition_data*))
  z_data_smem_app_a_partition_bss_start = .;
  KEEP(*(data_smem_app_a_partition_bss*))
  z_data_smem_app_a_partition_bss_end = .;
  . = ALIGN(_region_min_align); . = ALIGN( 1 << LOG2CEIL(z_data_smem_app_a_partition_bss_end - z_data_smem_app_a_partition_part_start));
  z_data_smem_app_a_partition_part_end = .;
  . = ALIGN(_region_min_align);;
  _app_smem_end = .;
 } > RAM AT > FLASH
 z_data_smem_shared_partition_part_size = z_data_smem_shared_partition_part_end - z_data_smem_shared_partition_part_start;
 z_data_smem_shared_partition_bss_size = z_data_smem_shared_partition_bss_end - z_data_smem_shared_partition_bss_start;
 ...
 z_data_smem_app_b_partition_part_size = z_data_smem_app_b_partition_part_end - z_data_smem_app_b_partition_part_start;
 z_data_smem_app_b_partition_bss_size = z_data_smem_app_b_partition_bss_end - z_data_smem_app_b_partition_bss_start;
 z_data_smem_app_a_partition_part_size = z_data_smem_app_a_partition_part_end - z_data_smem_app_a_partition_part_start;
 z_data_smem_app_a_partition_bss_size = z_data_smem_app_a_partition_bss_end - z_data_smem_app_a_partition_bss_start;
